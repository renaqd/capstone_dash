...
def get_pie_chart(entered_site):
    ls_df = spacex_df.groupby(['Launch Site'], as_index=False).mean()
    if entered_site == 'ALL':
        return px.pie(ls_df, values='class', names='Launch Site', title='Total Success Launches For All Sites')
    # return the outcomes in pie chart for a selected site
    ls_df = spacex_df[spacex_df['Launch Site'] == entered_site]
    ls_df['outcome'] = ls_df['class'].apply(lambda x: 'Success' if (x == 1) else 'Failure')
    ls_df['counts'] = 1
    return px.pie(ls_df, values='counts', names='outcome', title='Total Success Launches For Site ' + entered_site)
# TASK 4:
...
def get_scatter_chart(entered_site, slider):
    ...
    if entered_site == 'ALL':
        return px.scatter(ls_df,
                          x='Payload Mass (kg)', y='class',
                          color='Booster Version Category',
                          title='Correlation Between Payload and Launch success')
    # return the outcomes piechart for a selected site
    ls_df = ls_df[ls_df['Launch Site'] == entered_site]
    ls_df['outcome'] = ls_df['class'].apply(lambda x: 'Success' if (x == 1) else 'Failure')
    ls_df['counts'] = 1
    return px.scatter (ls_df,
                       x='Payload Mass (kg)', y='class',
                       color='Booster Version Category',
                       title='Correlation Between Payload and Launch Success for Site ' + entered_site)
